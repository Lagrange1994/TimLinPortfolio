...
export default function BackButton({ prefix, label, onClick }) {
    return (
        <button onClick={onClick} className={`back-btn pointer-events-auto flex items-center justify-center h-10 w-10 bg-border/10 backdrop-blur-md border border-border/10 rounded-full text-text/80 hover:text-text hover:border-${prefix}-primary/50 hover:bg-${prefix}-dark-lighter transition-all duration-300 shadow-lg group overflow-hidden hover:w-40`}>
