            ...
            return (
                <React.Fragment>
                    ...
                    <nav className="fixed top-0 left-0 w-full z-50 px-6 py-6 flex justify-between items-center pointer-events-none">
                        ...
                        <ThemeToggle prefix="tn" />
                    </nav>

                    <ScrollTopButton prefix="tn" visible={showBackToHero} onClick={() => scrollToSection(0)} />

                    <div id="main-scroller" ref={mainContainerRef}>
                        <section ref={heroRef} className="snap-section active-section flex items-center justify-center relative overflow-hidden hero-bg-custom">
                            <div className="container max-w-7xl mx-auto px-8 z-20">
                                <div className="max-w-5xl text-left">
                                    <div className={`inline-block px-4 py-1 rounded-full border hero-badge text-xs font-bold tracking-widest mb-6 ${loading ? 'opacity-0' : 'fade-in-up'}`} style={{ animationDelay: '0.1s', '--hero-badge-color': '#2DD4BF' }}>SMART CITY & PUBLIC SAFETY</div>
                                    <h1 className={`text-5xl lg:text-7xl font-heading font-black mb-8 leading-tight text-white drop-shadow-2xl text-left ${loading ? 'opacity-0' : 'fade-in-up'}`} style={{ animationDelay: '0.2s' }}>{t('title_main')}<br /><span className="text-transparent bg-clip-text font-heading" style={{ backgroundImage: 'linear-gradient(to right, #2DD4BF, #38BDF8)' }} dangerouslySetInnerHTML={{ __html: t('title_sub').replace('\n', '<br/>') }}></span></h1>
                                    ...
                                    <div ref={swipeContentRef} className="p-4 lg:p-8 pb-24">
                                        ...
                                        {activeTab === 'process' && (
                                            <div className="space-y-6 lg:space-y-8 animate-fadeIn">
                                                <h3 className="text-lg md:text-2xl font-bold text-text mb-2">{t('process_title')}</h3>
                                                <p className="text-xs text-text/60 mb-6">{t('process_sub')}</p>
                                                <ProcessTimeline
                                                    prefix="tn"
                                        ...
                                        {activeTab === 'solution' && (
                                            <div className="space-y-6 lg:space-y-8 animate-fadeIn">
                                                <h3 className="text-lg md:text-2xl font-bold text-text mb-2 lg:mb-6">{t('solution_title')}</h3>
