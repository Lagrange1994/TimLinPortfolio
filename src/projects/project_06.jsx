...
// --- FRAMES ---
        const PhoneFrame = ({ src, alt }) => (
            <div className="flex items-center justify-center w-full h-full pointer-events-none">
                <div className="relative h-full w-auto max-w-full aspect-[9/19] border-[6px] md:border-[8px] border-[#2d2d2d] max-lg:rounded-[18.57%/8.8%] lg:rounded-[2.5rem] overflow-hidden shadow-2xl bg-black animate-fadeIn shrink-0 pointer-events-auto">
        ...
        const TRANSLATIONS = {
            zh: {
                ...
                feat_2_title: "即時自助服務", feat_2_desc: "新增「常見問答」入口，提供第一線自助解決方案。",
                ...
                feat_5_title: "明確的行動呼籲", feat_5_desc: "內頁底部置入顯眼「我要領取」按鈕，引導下一步。",
                feat_6_title: "彈出式領取機制", feat_6_desc: "點擊後即時彈出條碼或優惠碼，減少跳轉等待。",
                gallery_title: "所有介面總覽",
        ...
        const App = () => {
            ...
            useEffect(() => {
                ...
                Promise.race([heroImagePromise, forceTimeout]).then(() => { setLoaderDone(true); setLoading(false); });
                                ...
                                <div className="w-full h-full flex flex-col glass-panel relative min-h-0">
                                    <div className="sticky top-0 bg-tym-dark/95 backdrop-blur-xl z-30 border-b border-border/10 shrink-0">
                                        <div className="p-4 lg:p-8 pb-0 lg:pb-0">
                                            ...
                                            <p className="text-text/60 text-xs lg:text-sm mb-2 lg:mb-4">桃園捷運 APP 改版計畫・體驗升級與商業整合策略</p>
                                            <TabNav
                                                prefix="tym"
                                                activeTab={activeTab}
