...
// --- COMPONENTS ---
        ...
        const TRANSLATIONS = {
            zh: {
                ...
                context_title: "專案背景", context_desc: "這是一個專為新竹市動物園內部員工與志工設計的整合管理系統。目標是將龐大的園區營運數據（動物照護、志工排班、人流安全）進行高效整合，並延伸至行動端以應對現場突發狀況。",
                role_title: "My Role", role_name: "UI 設計師",
                pain_title: "管理痛點", pain_sub: "轉型前的困境",
                ...
                web_title: "網頁後台", web_summary: "決策與分析司令部",
                web_core: "核心功能",
                web_p1: "全盤數據：人流長期趨勢分析、歷史報表。",
                web_p2: "深度管理：排班管理、知識庫建檔、系統設定。",
                web_p3: "適用情境：辦公室內的行政作業與策略規劃。",
                app_title: "手機 APP", app_summary: "現場執行與應變先鋒",
                app_core: "行動亮點",
                app_p1: "即時資訊：現場人流熱區圖、當日活動。",
                app_p2: "緊急應變：專屬緊急協助按鈕 (長按3秒)。",
                app_p3: "適用情境：巡邏、現場導覽、突發狀況處理。",
                gallery_title: "介面總覽", gallery_desc: "完整系統介面展示"
            },
            en: {
                        ...
                        <section ref={heroRef} className="snap-section active-section flex items-center justify-center relative hero-bg-custom">
                            <div className="container max-w-7xl mx-auto px-8 z-20">
                                <div className="max-w-5xl text-left">
                                    <div className={`inline-block px-4 py-1 rounded-full border hero-badge text-xs font-bold tracking-widest mb-6 ${loading ? 'opacity-0' : 'fade-in-up'}`} style={{ animationDelay: '0.1s', '--hero-badge-color': '#FFC75F' }}>SMART ZOO MANAGEMENT</div>
                                    <h1 className={`text-5xl lg:text-7xl font-heading font-black mb-8 leading-tight text-white drop-shadow-2xl text-left ${loading ? 'opacity-0' : 'fade-in-up'}`} style={{ animationDelay: '0.2s' }}>{t('title_main')}<br /><span className="text-transparent bg-clip-text font-heading" style={{ backgroundImage: 'linear-gradient(to right, #86C232, #FF9671)' }} dangerouslySetInnerHTML={{ __html: t('title_sub').replace('\n', '<br/>') }}></span></h1>
                                ...
                                <div className="w-full h-full flex flex-col glass-panel relative min-h-0">
                                    <div className="sticky top-0 bg-zoo-dark/95 backdrop-blur-xl z-30 border-b border-border/10 shrink-0">
                                        <div className="p-4 lg:p-8 pb-0 lg:pb-0">
                                            ...
                                            <p className="text-text/60 text-xs lg:text-sm mb-2 lg:mb-4">動物園內部管理與行動應變系統</p>
                                            <TabNav
                                                prefix="zoo"
