...
export default function BeamsBackground() {
  ...
  useEffect(() => {
    ...
    function scheduleInit(callback: () => void) {
      if ('requestIdleCallback' in window) {
        (window as any).requestIdleCallback(callback, { timeout: 2500 });
      } else {
        (window as Window & typeof globalThis).addEventListener('load', () => { setTimeout(callback, 800); }, { once: true });
      }
    }
    ...
    scheduleInit(async () => {
      ...
      const canvas = canvasRef.current || document.getElementById('beams-bg') as HTMLCanvasElement;
      if (!canvas) return;
      ...
      function extendMaterial(BaseMaterial: typeof THREE.MeshStandardMaterial, cfg: any) {
        ...
        if ((defaults as any).color) uniforms.diffuse.value = (defaults as any).color.clone();
...
gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`,
      ...
      function updateBg() {
        ...
        if (splineEl) splineEl.style.opacity = (1 - p).toFixed(3);
    ...
    return () => {
      cancelled = true;
      const canvas = canvasRef.current || document.getElementById('beams-bg') as HTMLCanvasElement;
      if (canvas && (canvas as any).__beamsCleanup) {
        (canvas as any).__beamsCleanup();
      }
    };
  }, []);
  ...
  return (
    <>
      <canvas id="beams-bg" ref={canvasRef} />
