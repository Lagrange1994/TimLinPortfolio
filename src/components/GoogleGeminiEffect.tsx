...
// trimmed to just the animated beam SVG (no hero title/CTA) and recolored to the
// site's existing purple->cyan AI gradient (see .ai-principle-dot) instead of
// Google's pink/orange/blue. All 5 original strands are kept — evenly spaced
// (~19.6% apart, see GEMINI_BEAM_STOPS) — one per AI category chip in the
// Skills section, each chip pinned to its own beam's right-edge y.
const PATHS = [
...
const COLORS = ['#8A2BE2', '#7A3FE8', '#5A6EF0', '#2FA0FA', '#00D4FF'];

// SVG viewBox window (y 320-700) — wide enough that every kept path stays
...
interface BeamStop {
  color: string;
  // Vertical position of this beam's right-edge endpoint, as a %-from-top of
  // the SVG's rendered height. Since the SVG stretches to exactly fill this
  // component's box (preserveAspectRatio="none"), a chip placed at this same
  // %-from-top of the container sits right on the line.
  topPercent: number;
}

// Ordered top-to-bottom to match the reading order of the AI category chips
// in SkillsSection (Dev Handoff / Product UI / Graphic Design / UX Review /
// Research Planning) — index-paired 1:1 so each chip's vertical center lands
// on its beam's endpoint y.
export const GEMINI_BEAM_STOPS: BeamStop[] = [
  { color: '#00D4FF', topPercent: ((364 - VIEWBOX_MIN_Y) / VIEWBOX_HEIGHT) * 100 },
  { color: '#2FA0FA', topPercent: ((439 - VIEWBOX_MIN_Y) / VIEWBOX_HEIGHT) * 100 },
  { color: '#5A6EF0', topPercent: ((513.235 - VIEWBOX_MIN_Y) / VIEWBOX_HEIGHT) * 100 },
  { color: '#7A3FE8', topPercent: ((588 - VIEWBOX_MIN_Y) / VIEWBOX_HEIGHT) * 100 },
  { color: '#8A2BE2', topPercent: ((662.5 - VIEWBOX_MIN_Y) / VIEWBOX_HEIGHT) * 100 },
];
...
export default function GoogleGeminiEffect({ className, onArriveChange }: GoogleGeminiEffectProps) {
  ...
  useEffect(() => {
    if (reduceMotion) {
      ...
      onArriveChange?.([true, true, true, true, true]);
    }
  }, [reduceMotion, onArriveChange]);
